...
public class AdminRecRepListController {
	...
	public HashMap<String, Object> RecRepLoad(@RequestParam int page){
		...
		if(row_RecRep==0) {
			...
			int totalPages=row_RecRep/10;
			if(row_RecRep%10!=0) {
				totalPages++;
			}
			ArrayList<DecListDTO> RecRepList = arrlDAO.RecRepList(offset);
	...
	public HashMap<String, Object> stateRepSearch(@RequestParam String state, @RequestParam int page){
		HashMap<String, Object> map = new HashMap<String, Object>();
		int row_RecRep = arrlDAO.searchRepCnt(state);
		dateStrs = new ArrayList<String>();
		int offset = (page-1)*10;
		logger.info(state);
		if(row_RecRep==0) {
			...
			map.put("total",totalPages);
			ArrayList<DecListDTO> searchRepList = arrlDAO.searchRepList(state,offset);
